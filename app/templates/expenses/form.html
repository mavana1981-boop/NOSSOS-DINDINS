...
<div class="card" style="max-width:780px;">
  <form method="POST" id="expense-form">
    ...
    <div class="form-row">
      <div class="form-group">
        ...
        <input class="form-control mono" name="amount" id="amount-input" required inputmode="decimal"
               value="{{ fmt(expense.amount) if expense else '' }}"
               placeholder="0,00">
      </div>
      <div class="form-group">
    ...
    <h3 style="margin-bottom:12px;">Tipo de gasto</h3>

    <div class="form-group">
      <div class="grid grid-2" style="gap:12px;">
        <label class="form-check kind-option" data-kind="pontual">
          <input type="radio" name="kind" value="pontual"
                 {% if not expense or expense.kind == 'pontual' %}checked{% endif %}>
          <div>
            <strong>Pontual</strong>
            <div class="text-faint text-small">Único, na data informada.</div>
          </div>
        </label>
        <label class="form-check kind-option" data-kind="recorrente">
          <input type="radio" name="kind" value="recorrente"
                 {% if expense and expense.kind == 'recorrente' %}checked{% endif %}>
          <div>
            <strong>Recorrente / Parcelado</strong>
            <div class="text-faint text-small">Repete todo mês a partir da data inicial.</div>
          </div>
        </label>
      </div>
    </div>

    <div id="kind-recorrente-extra" class="form-group" style="display:none;">
      <label class="form-label">Quantidade de meses</label>
      <input class="form-control" type="number" name="recurrence_months" min="1" max="360"
             value="{{ expense.recurrence_months if expense and expense.recurrence_months else '' }}"
             placeholder="ex: 12 (deixe vazio para sem prazo)">
      <div class="form-help">
        Deixe vazio para gasto fixo perpétuo (luz, aluguel).
        Informe o número para parcelado (ex: 10 vezes no cartão).
      </div>
    </div>
        ...
        {% for u in users %}
          ...
          <div class="share-row">
            ...
            <input class="form-control mono split-input" name="share_user_{{ u.id }}"
                   ...
                   value="{{ fmt(existing_share.amount) if existing_share.amount else '' }}" placeholder="0,00">
          </div>
        {% endfor %}
      </div>
...
(function() {
  ...
  const recExtra = document.getElementById('kind-recorrente-extra');
  ...
  function updateKind() {
    ...
    recExtra.style.display = k === 'recorrente' ? 'block' : 'none';
  }

  radios.forEach(r => r.addEventListener('change', updateMode));
  kindRadios.forEach(r => r.addEventListener('change', updateKind));
